		...
		return {
			...
			getTableWidthTemplate: function()
			{
				...
				if( this.lang.get('table_col_width_label') == '' || typeof this.lang.get('table_col_width_label') == 'undefined'){
					var table_col_width_label = 'По умолчанию в px,<br>можно добавить %'; // Yes, this is Dutch - Robin
				}else{
				...
				+ '<section id="redactor-modal-table-col-width">'
					+	'<label style="line-height: 1.4; margin: 0 0 8px !important">'+table_col_width_label+'</label>'
			...
			init: function()
			{
				...
				var button = this.button.addAfter('link', 'table', this.lang.get('table'));
			...
			editTableWidths: function(){
				...
				if( this.lang.get('table_col_width_modal_heading') == '' || typeof this.lang.get('table_col_width_modal_heading') == 'undefined'){
					var table_col_width_modal_heading = 'Ширина столбца';
				}else{
				...
				if( this.lang.get('apply') == '' || typeof this.lang.get('apply') == 'undefined'){
					var table_col_width_modal_apply_button = 'Применить';
				}else{
				...
				this.modal.show();
				$('#redactor-table-col-width').val($('.redactor-selection-marker').closest('td')[0].style.width);
				$('#redactor-table-col-width').focus();
			},
			applyWidth: function(){
				// translation
				if( this.lang.get('is_not_a_valid_value') == '' || typeof this.lang.get('is_not_a_valid_value') == 'undefined'){
					var is_not_a_valid_value = 'Недопустимое значение ';
				}else{
				...
				if(parseInt(new_column_width_value) < 1 || parseInt(new_column_width_value) > 1280 || isNaN(parseInt(new_column_width_value))){ // 1280 wide is enough I think - Robin
					alert(is_not_a_valid_value + new_column_width_value);
				}else{
					...
					var $current = $(this.selection.getCurrent());
          $current = $current.prop('tagName') == 'td' ? $current : $current.closest('td');
